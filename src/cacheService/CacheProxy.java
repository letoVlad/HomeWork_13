...
public class CacheProxy implements InvocationHandler {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
        if (method.isAnnotationPresent(Cache.class)) {
            ...
            if (cache.containsKey(key)) { // Ищем результат в памяти
                return cache.get(key);
            }

            if (cacheAnnotation.cacheType() == CacheType.FILE) { // Ищем результат в файле
            ...
            switch (cacheAnnotation.cacheType()) { // Сохранить результат в кэш
                case IN_MEMORY -> saveToMemory(method, args, key);
     ...
     */
    private void saveToMemory(Method method, Object[] args, String key) throws InvocationTargetException, IllegalAccessException {
        ReentrantLock lock = locks.computeIfAbsent(key, k -> new ReentrantLock());
        lock.lock();
        try {
            if (!cache.containsKey(key)) { // Проверяем еще раз, чтобы избежать дублирования
    ...
    private void saveToFile(Method method, Object[] args, String key) throws InvocationTargetException, IllegalAccessException {
        ...
        lock.lock();
        try {
            if (!cache.containsKey(key)) { // Проверяем еще раз
                Object result = method.invoke(target, args);
                File outputFileName = getFile(method, key, method.getName());
